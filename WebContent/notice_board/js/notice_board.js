$(function() {
	var URL = window.location.protocol+"//"+window.location.host+"/notice_board/";

	//페이지에 맞는 리스트 보기
	$('.writing input').click(function() {
		var fullurl = URL+"notice_board/writing.jsp";
	...
	$.eventblock = function(page) {
			...
			$.ajax({
				type: "get",
				async: true,
				url: URL+"boardList.bo?Handler=ajax",
				data: {"page" : page},
				dataType: "text",
				success: function(data) {
					...
					for(var i in memberlist){
						...
						list += 	"<img src='/notice_board/images/reply/blank.gif' alt='화살표이미지'> ";		
							}
						list += 	"<img src='/notice_board/images/reply/arrow.png' alt='화살표이미지'> ";
						}
	...
	$('.left input').on('click', function() {
		window.location.href = URL+"notice_board/division.jsp?arrow=left";
	});
	
	$('.right input').on('click', function() {
		window.location.href = URL+"notice_board/division.jsp?arrow=right";
	});
